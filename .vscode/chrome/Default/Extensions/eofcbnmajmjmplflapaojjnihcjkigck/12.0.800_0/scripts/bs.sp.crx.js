...
(function (_) {
    ...
    function urlInfoChange(url, tab, change, event) {
        var referer = AvastWRC.TabReqCache.get(tab.id,"referer");

        var couponInTabsToShow = AvastWRC.UtilsCache.get("coupons_tabs_to_show", tab.id);
        if(couponInTabsToShow){
            if(couponInTabsToShow.coupons
                && (couponInTabsToShow.toBeShownIn[tab.url] || couponInTabsToShow.toBeShownIn[referer] || AvastWRC.bal.sp.siteNameIsContainedOnCurrentURL(tab.url, couponInTabsToShow.merchantURL))){
        ...
        AvastWRC.bs.tabExists.call(this, info.tabId, function () {
            chrome.tabs.get(info.tabId, function (tab) {
                ...
                if(couponInTabsToShow){
                    ...
                    console.log("onRedirect-> RequestId: " + info.requestId + " code: " + info.statusCode + " Initiator: " + info.initiator +  " REDIRECT from " + info.url + " to " + info.redirectUrl);
                    console.log(info);
                    console.log(couponInTabsToShow);
                    console.log("---------------------------REDIRECT---------------------------");   
                    ...
                    if(!AvastWRC.Utils.getBrowserInfo().isFirefox()){  
                        ...
                        if(info.originUrl == undefined && (couponInTabsToShow.toBeShownIn && !couponInTabsToShow.toBeShownIn[info.url])){
                            console.log("Remove here the coupon url: " + info.url + "urlto" + info.redirectUrl);
                            AvastWRC.UtilsCache.remove("coupons_tabs_to_show", tab.id);
                        }
                    }
    ...
    function onClicked(tab){
        AvastWRC.bs.tabExists.call(this, tab.id, function () {
            ...
            eventDetails.clientInfo.transaction_id = "";
            ...
            if(closedCoupon && tab.url.indexOf(closedCoupon.closedCouponUrl) != -1){
                console.log(closedCoupon.closedCouponUrl);
                ...
                AvastWRC.bal.emitEvent("control.show", tab.id);
                AvastWRC.bal.emitEvent("control.setIcon", tab.id, "common/ui/icons/logo-safeprice-128.png");
                eventDetails.clientInfo.transaction_id = closedCoupon.transactionId || "";
                ...
                eventDetails.clientInfo.transaction_id = cachedData.transactionId || "";
                console.log(eventDetails);
                (AvastWRC.Burger != undefined && cachedData.badgeHighlighted) ? AvastWRC.Burger.emitEvent("burger.newEvent", eventDetails) : console.log("no burger lib or no badge highlighted");
            }else{
