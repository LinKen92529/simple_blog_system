    ...
    format: function () {
        ...
        if (resp != undefined && typeof resp === "object" && resp != null) {
            ...
            if(resp.redirect && resp.redirect.redirect.length > 0){
                console.log("onlyFirstRequest -> there was a redirect");
                res.onlyFirstRequest = true;
                    ...
                    if (redirects && redirects.length > 0) {
                        ...
                        for (var m = 0, n = redirects.length; m < n; m++) {
                            ...
                            if (redirect != undefined) {
                                var redirectImages = AvastWRC.Utils.resolveImageUrl(redirect.primary_image || "", redirect.secondary_image || "");
                                var redDetails = {
                                    label: redirect.title || "",
                                    url: redirect.url || "",
                                    image: redirect.image || AvastWRC.bs.getLocalImageURL("deals.png"),
